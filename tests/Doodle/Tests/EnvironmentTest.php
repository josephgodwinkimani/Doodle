<?php

declare(strict_types=1);

/*
 * This file is part of the Doodle package.
 *
 * (c) Godwin Kimani <josephgodwinke@gmail.com>
 *
 * For the full copyright and license information, please view the LICENSE
 * file that was distributed with this source code.
 */

namespace Doodle\Tests;

use PHPUnit\Framework\TestCase;
use Doodle\Environment;

class EnvironmentTest extends TestCase
{
    public function testGetEnvironmentVariable()
    {
        // Create an instance of the Environment class
        $environment = new Environment();

        // Test the get method
        $this->assertEquals("Doodle", $environment->get("APP_NAME"));
    }

    public function testGetEnvironmentVariableForTesting()
    {
        // Create an instance of the Environment class
        $environment = new Environment();

        // Test the get method
        $this->assertEquals("Doodle", $environment->get("APP_NAME"), "test");
    }
}
